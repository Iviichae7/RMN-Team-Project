      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {plans.map((plan) => (
              <div
                key={plan.name}
                className="flex flex-col bg-gray-600 bg-opacity-20 backdrop-filter backdrop-blur-lg p-6 rounded-lg border border-gray-300 border-opacity-50 shadow-lg transform transition-transform duration-300 hover:scale-105"
              >
        ...
        {selectedPlan && (
          <div className="mt-8 text-center">
            <h3 className="text-2xl font-bold">You have selected:</h3>
            <p className="mt-2 text-xl">{selectedPlan.name} Plan</p>
          </div>
        )}
      </div>
    </div>
